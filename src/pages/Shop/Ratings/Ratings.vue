...
<script>
  ...
  export default {
    ...
    data() {
      return {
        onlyShowText: false, // 是否只显示有内容文本的
        selectType: -1 // -1: 全部, 0: 推荐, 1: 吐槽
      }
    },
    ...
    mounted() {
      ...
      // 卸载该组件时，在事件总线上移除自定义事件监听器
      this.$once('hook:beforeDestroy', () => {
        this.$eventBus.$off('setSelectType', this.setSelectType)
        this.$eventBus.$off('toggleOnlyText', this.toggleOnlyText)
      })

      if (this.ratings.length > 0) { // 从其它路由切换过来, 数据已经有了
        this._initScroll()
      }
    },
