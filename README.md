...
## 5. 打包文件大小可视化界面分析与优化
    1). 打包并且自动生成可视化结构页面
        命令: npm run build --report 或 yarn build --report
        其中: 所占比越大代表占用性能(体积)越大,考虑是否优化
        这样打包完成之后在打包后的文件夹中会给我们生成一个report.html文件,我们打开该文件之后，就会看到一个可视化的文件占比图，根据哪个文件的占比 比较大，我们就可以对那个文件进行优化操作
