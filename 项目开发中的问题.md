# 1. 使用vue-cli3脚手架遇到的坑
    1). 问题: 异常: You are using the runtime-only build of Vue where the template compiler is not available
        原因: 默认引用的vue包是不带template模板编译器的vue.runtime.esm.js, 而我们需要带template模板编译器的版本:vue.esm.js
        ...
        解决2: 配置指定引用带编译器的版本:vue.esm.js   --vue.config.js
            module.exports = {
              configureWebpack: {
                resolve: {
                  alias: {
                    'vue$': 'vue/dist/vue.esm.js'  // $表示精准匹配,即import Test1 from 'vue';时是精确匹配，所以 vue/dist/vue.esm.js被解析和导入;但import Test2 from 'vue/file.js';时是非精确匹配，触发普通解析，vue/file.js被解析和导入。 vue/dist/vue.esm.js中的vue.esm.js是带template模板编译器的
                  }
                }
              }
            }
